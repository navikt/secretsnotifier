...
fun main() = runBlocking {
    ...
    mutableMapOf<Team, List<RepoWithSecret>>().apply {
        ...
        val msg = "GitHub har oppdaget hemmeligheter i repo som dere er admin i: ${linksTo(repos)}. Klikk på linkene for å se detaljer. Dersom hemmelighetene er aktive må de roteres så fort som mulig, og videre varsling og steg for å avdekke evt. misbruk må iverksettes. Når dette er gjort (eller dersom dette er falske positiver) lukkes varselet ved å velge i nedtrekksmenyen `Close as`."
...
private fun envOrDie(name: String): String = getenv(name)?.toKString()
    ?: throw RuntimeException("Unable to find envvar $name, I'm useless without it")
